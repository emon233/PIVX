...
"""Test lynxd with different proxy configuration.

Test plan:
- Start lynxd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on lynxd side:
...
- Create lynxds that connect to them
- Manipulate the lynxds using addnode (onetry) an observe effects
...
class ProxyTest(LynxTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: lynxd's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
