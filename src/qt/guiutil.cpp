...
// Copyright (c) 2015-2021 The LYNX Core developers
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter LYNX address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // Convert lynx:// to lynx:
    //
    //    Cannot handle this later, because lynx:// will cause Qt to see the part after // as host,
...
bool openConfigfile()
{
    return openFile(GetConfigFile(gArgs.GetArg("-conf", LYNX_CONF_FILENAME)), true);
}
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        ...
        return GetSpecialFolderPath(CSIDL_STARTUP) / "LYNX (regtest).lnk";
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=LYNX (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=LYNX (regtest)\n";
        else
