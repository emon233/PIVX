...
#include "qt/lynx/forms/ui_snackbar.h"
#include "qt/lynx/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(LYNXGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &LYNXGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
