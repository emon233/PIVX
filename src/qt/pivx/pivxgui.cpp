...
const QString LYNXGUI::DEFAULT_WALLET = "~Default";

LYNXGUI::LYNXGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void LYNXGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &LYNXGUI::toggleHidden);
...
void LYNXGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &LYNXGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &LYNXGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &LYNXGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &LYNXGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &LYNXGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &LYNXGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &LYNXGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &LYNXGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &LYNXGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &LYNXGUI::execDialog);
    connect(governancewidget, &GovernanceWidget::showHide, this, &LYNXGUI::showHide);
    connect(governancewidget, &GovernanceWidget::execDialog, this, &LYNXGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &LYNXGUI::execDialog);
}


void LYNXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void LYNXGUI::handleRestart(QStringList args)
{
...
void LYNXGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        connect(clientModel, &ClientModel::message, this, &LYNXGUI::message);
...
void LYNXGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &LYNXGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &LYNXGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void LYNXGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void LYNXGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void LYNXGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &LYNXGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &LYNXGUI::show);
                e->ignore();
            }
        }
    }
...
void LYNXGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void LYNXGUI::messageInfo(const QString& text)
{
...
void LYNXGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
bool LYNXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
...
void LYNXGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void LYNXGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void LYNXGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void LYNXGUI::goToSend()
{
    showTop(sendWidget);
}

void LYNXGUI::goToAddresses()
{
    showTop(addressesWidget);
}

void LYNXGUI::goToMasterNodes()
{
    masterNodesWidget->resetCoinControl();
    showTop(masterNodesWidget);
}

void LYNXGUI::goToColdStaking()
{
    showTop(coldStakingWidget);
}

void LYNXGUI::goToGovernance()
{
    showTop(governancewidget);
}

void LYNXGUI::goToSettings(){
    showTop(settingsWidget);
}

void LYNXGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void LYNXGUI::goToReceive()
{
    showTop(receiveWidget);
}

void LYNXGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void LYNXGUI::showTop(QWidget* view)
{
...
void LYNXGUI::changeTheme(bool isLightTheme)
{
...
void LYNXGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool LYNXGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void LYNXGUI::showHide(bool show)
{
...
int LYNXGUI::getNavWidth()
{
    return this->navMenu->width();
}

void LYNXGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
void LYNXGUI::setGovModel(GovernanceModel* govModel)
{
    if (!stackedContainer || !clientModel) return;
    governancewidget->setGovModel(govModel);
}

void LYNXGUI::setMNModel(MNModel* _mnModel)
{
    if (!stackedContainer || !clientModel) return;
    mnModel = _mnModel;
...
bool LYNXGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(walletModel, &WalletModel::message, this, &LYNXGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &LYNXGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &LYNXGUI::message);
    connect(topBar, &TopBar::message, this, &LYNXGUI::message);
    connect(sendWidget, &SendWidget::message,this, &LYNXGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &LYNXGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &LYNXGUI::message);
    connect(governancewidget, &GovernanceWidget::message,this, &LYNXGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &LYNXGUI::message);

    // Pass through transaction notifications
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &LYNXGUI::incomingTransaction);

    return true;
}

bool LYNXGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void LYNXGUI::removeAllWallets()
{
    // Single wallet supported.
}

void LYNXGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(LYNXGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void LYNXGUI::subscribeToCoreSignals()
{
...
void LYNXGUI::unsubscribeFromCoreSignals()
{
