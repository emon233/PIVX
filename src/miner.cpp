...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("LYNXMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("LYNXMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("LYNXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("lynx-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running LYNXMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("LYNXMiner exception");
    } catch (...) {
        LogPrintf("LYNXMiner exception");
    }

    LogPrintf("LYNXMiner exiting\n");
}
